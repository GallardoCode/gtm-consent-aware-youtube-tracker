...
The maintainer imported the corrected template and confirmed all six editor tests passing on 2026-09-12 at 13:53:42, as displayed by GTM, followed by a successful Run Code check at 13:55:03. In container Preview, the maintainer confirmed denied startup, later grants, repeated pause/resume, withdrawal, and regrant in both Tag Assistant and the demo panel. Repeated tag execution produced no duplicate events, and a subsequent pause/resume produced one new event. The saved-grant/ordinary-host URL also tracked immediately and respected withdrawal and regrant. A further report about regrant during active playback was traced to the demo skipping GTM-wrapped messages; the maintainer has now confirmed the wrapper reader fix below works in Preview. Detailed network/cookie observations and separate CMP integration checks have not been supplied. Their prepared procedure is `bash scripts/verify-gtm.sh`.
...
## Regrant during active playback and event visibility, 2026-09-12

After confirming repeated execution and saved-grant checks, the maintainer reported a missing event when granting consent during active playback after a withdrawal. On clarification, the event was present in Tag Assistant and missing only from the demo display. No tracker failure was reproduced. Regrant is intended to report the current position immediately when the player is already playing.

A real YouTube probe started playback while denied, granted, then repeated withdrawal, pause/resume while denied, and regrant three times on each embed host. Every regrant added exactly one event at the current playback position, and the panel matched the data layer. The initial probe used a broad Play selector that also matched Playback settings; tightening it to the actual Play control and checking for the Pause control avoided falsely treating a paused video as playing. These probes used the local GTM adapter, not the authenticated Preview session.

The demo previously appended new events below all older entries. To make repeated checks easier to read, it now displays the newest event first and an event count. This is a visibility improvement, not a confirmed fix for an event emission defect. The regression test first failed with displayed positions `[20, 70]` instead of `[70, 20]`, then passed after the display change. It also verifies that the data layer retains its original chronological order. The full suite passes with 7 template checks and 16 browser cases, and type checking passes. The template and pinned companion are unchanged; reload the demo to get the display change.

The maintainer retested the updated panel and still reported that regrant during playback appeared in Tag Assistant but not on the page. The visibility change did not resolve that report. At that point the discrepancy remained open and could not be classified as a scrolling issue or a verified tracker pass for this specific transition. A further public-GTM probe pushed playback messages during GTM message processing and found matching data-layer and displayed events. The investigation next requested a same-tab snapshot of raw playback events and rendered entries, paired with the exact regrant Message object from Tag Assistant. Further runtime changes waited for that comparison.

## GTM wrapper display fix, 2026-09-12

The maintainer supplied the decisive comparison: the panel and plain-event filter both contained three entries, ending at event ID 13 and video position 37 seconds, while Tag Assistant showed event ID 16 at 44 seconds. The rendered panel matched its filtered input. Inspecting Google's delivered runtime identified a different storage shape: synchronous sandbox-origin pushes are wrapped and expose the payload through `getUntrustedMessageValue()`. Filtering only a top-level `event` skips those messages. The earlier plain-array diagnostics also skipped them.

An isolated public-GTM probe exercised the runtime's real wrapper code and reproduced the exact omission: an ordinary event at 37 seconds appeared, while the wrapped event at 44 seconds was processed but omitted from the panel. The probe temporarily changed the sandbox semaphore and restored it; no such change is part of the demo or tracker. The browser regression then failed for wrapped regrant messages, showing `[20]` instead of `[70, 20]`, while its plain-message case passed.

The demo now reads a wrapper's payload before filtering. The same real-GTM probe and both regression cases pass. Original wrapped messages and chronological data-layer order are preserved. The full suite passes with 7 template checks and 17 browser cases, and type checking passes. The production companion, exported template, and consent timing are unchanged. After reloading the demo, the maintainer confirmed on 2026-09-12 that the fix was working perfectly in their authenticated Preview session. This completes the reported playback and consent walkthrough, including the immediate regrant event appearing in both displays. Runtime findings are saved in [the GTM research notes](research/gtm-community-template.md#gtm-wrapped-data-layer-messages-observed-2026-09-12).

The incremental standards and spec reviews of the wrapper display fix both reported zero findings. The implementation is ready for PR review; the broader first-release work remains in parent #1.
