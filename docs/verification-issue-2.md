...
The maintainer retested the updated panel and still reported that regrant during playback appeared in Tag Assistant but not on the page. The visibility change did not resolve that report. The discrepancy remains open; it must not be classified as a scrolling issue or a verified tracker pass for this specific transition. A further public-GTM probe pushed playback messages during GTM message processing and found matching data-layer and displayed events. The next evidence requested is a same-tab snapshot of raw playback events and rendered entries, paired with the exact regrant Message object from Tag Assistant. No further runtime change has been made without that comparison.
