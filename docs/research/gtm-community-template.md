...
### GTM-wrapped data-layer messages, observed 2026-09-12

Inspecting Google's delivered `gtm.js` and running it in an isolated browser established that a data-layer push during sandbox execution can be stored in a wrapper exposing `getUntrustedMessageValue()`. The payload's `event` field is inside that wrapper. GTM processes the payload, and Tag Assistant can show it, while a page diagnostic that filters only `item.event` silently skips it. This wrapper is observed runtime behavior, not a documented template API. Google's documented [callInWindow bridge](https://developers.google.com/tag-platform/tag-manager/templates/api#callinwindow) invokes page functions from the sandbox; its documentation does not describe this storage detail.

This distinction matters for the tracker demo. Initial player readiness and later playback callbacks run asynchronously, producing ordinary messages. Regrant while already playing can report synchronously inside the consent-to-companion bridge, producing a wrapped message. The production tracker still delivers the event to GTM. The demo reader must decode a wrapper before selecting playback events, while preserving the original array and wrapped messages. Do not change consent timing or repush the payload merely to make a diagnostic display see it.

A controlled public-GTM probe reproduced the omission with ordinary and wrapped events at 37 and 44 seconds. Before the reader fix, only 37 appeared in the panel; after the fix both appeared. The probe temporarily set GTM's sandbox semaphore in an isolated browser to exercise the real wrapping code, then restored it. It did not establish authenticated Preview behavior. `tests/demo.spec.js` models the same wrapping boundary during the named bridge call and checks initial display, regrant, later ordinary playback, event counts, and retained data-layer ordering. [Verification evidence](../verification-issue-2.md#gtm-wrapper-display-fix-2026-09-12)
